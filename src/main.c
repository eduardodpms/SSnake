...
int main(){
    ...
    int config[8] = {1, 0, 15, 4, 'O', 'C', '*', 0}; // Vetor de ajustes (configs)
    ...
    while(run){
        ...
        if(input=='1') while(flag++) game(&error, config, wait, mode, &flag, &best_score); // Função para o menu de jogo
        else if(input=='2') options_menu(&error, config, wait, mode); // Função para o menu de ajustes
